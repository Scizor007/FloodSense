...
export function LeafletMapInner({
  ...
  return (
    <div className={`relative h-full w-full overflow-hidden rounded-xl ${className}`}>
      ...
      <div className="pointer-events-auto absolute top-3 right-3 z-[400] w-64 rounded-xl border border-border/80 bg-card/90 p-3 shadow-xl backdrop-blur-md">
        ...
        <div className="space-y-1.5 text-xs">
          <label className="flex cursor-pointer items-center justify-between rounded-lg p-1.5 transition-colors hover:bg-secondary/60">
            <span className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={layers.predictedRisk}
                onChange={() => toggleLayer("predictedRisk")}
                className="rounded border-border accent-primary"
              />
              <span className="font-medium text-foreground">Predicted Flood Risk</span>
            </span>
            <span className="font-mono text-[9px] text-muted-foreground">
              {gridPoints?.length ?? 0} cells
            </span>
          </label>
          ...
          <label className="flex cursor-pointer items-center justify-between rounded-lg p-1.5 transition-colors hover:bg-secondary/60">
            ...
            <span className="font-mono text-[9px] text-cyan-600 dark:text-cyan-400">
              {validReports.length} live
            </span>
          </label>

          {/* Affected corridors (geometry unavailable in current backend) */}
          <div
            className="flex items-center justify-between rounded-lg p-1.5 opacity-60"
            title="Corridor vector line geometry is not yet supported by the backend. No synthetic geometry is rendered."
          >
            <span className="flex items-center gap-2">
              <input
                type="checkbox"
            ...
            <span className="font-mono text-[8.5px] uppercase text-amber-600 dark:text-amber-400">
              No geometry
            </span>
          </div>
        </div>
